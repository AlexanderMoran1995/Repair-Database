...
const long NervoPanel::idButtonNewRepair = ::wxNewId();   //New Repair Button
const long NervoPanel::idButtonRefreshGrid = ::wxNewId();   //Refresh Button
const long NervoPanel::idButtonGetInfo = ::wxNewId();   //Get Button
const long NervoPanel::idButtonSearch = ::wxNewId();   //Search Button
const long NervoPanel::idButtonUpdateRepair = ::wxNewId();   //Update Repair Button
const long NervoPanel::idButtonResetGrid = ::wxNewId();   //Reset Button
const long NervoPanel::idButtonSortGrid = ::wxNewId();   //Sort Button

//const long NervoPanel::idGridLeftClick = ::wxNewId();
//const long NervoPanel::idGridRightClick = ::wxNewId();

const long NervonewRepairDialog::idButtonCreateRepair = ::wxNewId();   //Create Button
const long NervonewRepairDialog::idButtonCancelRepair = ::wxNewId();   //Cancel Button
const long NervonewRepairDialog::idButtonRepairTemplate = ::wxNewId();   //Cancel Button

const long NervoupdateRepairDialog::idButtonDoRepairUpdate = ::wxNewId();   //Create Button
const long NervoupdateRepairDialog::idButtonCancelRepairUpdate = ::wxNewId();   //Cancel Button
const long NervoupdateRepairDialog::idButtonAddNote = ::wxNewId();   //Cancel Button
const long NervoupdateRepairDialog::idButtonSelect = ::wxNewId();   //Cancel Button


BEGIN_EVENT_TABLE(NervoPanel, wxPanel)

EVT_BUTTON(idButtonNewRepair, NervoPanel::onClickNewRepair) //Login Button
EVT_BUTTON(idButtonRefreshGrid, NervoPanel::onClickRefresh) //Login Button
EVT_BUTTON(idButtonGetInfo, NervoPanel::onClickGet) //New User Button
EVT_BUTTON(idButtonSearch, NervoPanel::onClickSearch) //Login Button
EVT_BUTTON(idButtonUpdateRepair, NervoPanel::onClickUpdateRepair) //Login Button
EVT_BUTTON(idButtonResetGrid, NervoPanel::onClickReset) //Login Button
EVT_BUTTON(idButtonSortGrid, NervoPanel::onClickSort) //Login Button

EVT_TIMER(TIMER_ID, NervoPanel::OnTimer)

EVT_GRID_CELL_LEFT_CLICK(NervoPanel::OnCellLeftClick)
EVT_GRID_CELL_RIGHT_CLICK(NervoPanel::OnCellRightClick)
EVT_GRID_CELL_LEFT_DCLICK(NervoPanel::OnCellLeftDoubleClick)

END_EVENT_TABLE()



BEGIN_EVENT_TABLE(NervonewRepairDialog, wxDialog)

EVT_BUTTON(idButtonCreateRepair, NervonewRepairDialog::onClickCreateRepair) //Login Button
EVT_BUTTON(idButtonCancelRepair, NervonewRepairDialog::onClickCancelRepair)
EVT_BUTTON(idButtonRepairTemplate, NervonewRepairDialog::onClickGetRepairTemplate)

END_EVENT_TABLE()



BEGIN_EVENT_TABLE(NervoupdateRepairDialog, wxDialog)

EVT_BUTTON(idButtonDoRepairUpdate, NervoupdateRepairDialog::onClickDoRepairUpdate) //Login Button
EVT_BUTTON(idButtonCancelRepairUpdate, NervoupdateRepairDialog::onClickCancelRepairUpdate)
EVT_BUTTON(idButtonAddNote, NervoupdateRepairDialog::onClickAddNote)
EVT_BUTTON(idButtonSelect, NervoupdateRepairDialog::onClickSelect)

END_EVENT_TABLE()


NervoBackgroundPanel::NervoBackgroundPanel(wxPanel* parent)
    : wxPanel(parent, -1, wxDefaultPosition,
        wxSize(1207, 707))
{

    NGP = new NervoPanel(this);

}


NervoBackgroundPanel::~NervoBackgroundPanel() {}

NervoPanel::NervoPanel(wxPanel* parent)
    : wxPanel(parent, -1, wxDefaultPosition,
        wxSize(1207, 707))
{
    ///PARENT PANEL
    ...
    m_fieldarray.Add("Customer Name");
    ...
    grid->SetColLabelValue(1, _("Doctor Name"));
    ...
    wxSQLite3ResultSet Res = db->ExecuteQuery("SELECT Id, ContactName, RepairNumber, ModelNumber, Status, DateSent, DateReceived FROM nodeData WHERE Status != 'Shipped' ORDER BY rowid ASC;");
    dbSize = 0;
    // grid->ClearGrid();
    ...
    for (int i = 0; i < 5; i++)
    {
        ...
        contactDataGrid->SetCellFont(i, 1, wxFont(12, wxSWISS, wxBOLD, wxBOLD, false, _("MS Sans Serif")));
    }
...
NervoPanel::~NervoPanel() {}





void NervoPanel::onClickNewRepair(wxCommandEvent& event)
{
    NervonewRepairDialog* NRP = new NervonewRepairDialog(wxT("New Repair"));
    NRP->Show(true);
}


void NervoPanel::REFRESH()
{
    int n = 0;
    ...
    if (dbQuery == "") { dbQuery = "SELECT Id, ContactName, RepairNumber, ModelNumber, Status, DateSent, DateReceived FROM nodeData WHERE Status != 'Shipped' ORDER BY rowid ASC;"; }
...
void NervoPanel::onClickRefresh(wxCommandEvent& event)
{
    REFRESH();
}


void NervoPanel::OnTimer(wxTimerEvent& event)
{
    REFRESH();
}


void NervoPanel::onClickGet(wxCommandEvent& event)
{
...
void NervoPanel::onClickSearch(wxCommandEvent& event)
{
    ...
    else
    {
        ...
        wxSQLite3ResultSet Res;
            ...
            if (getSearchCategory == "Customer Name") { dbQuery = "SELECT Id, ContactName, RepairNumber, ModelNumber, Status, DateSent, DateReceived FROM nodeData WHERE ContactName LIKE '%" + getSearchCriteria + "%' AND Status != 'Shipped' ORDER BY rowid DESC;"; }
       else if (getSearchCategory == "Repair Number") { dbQuery = "SELECT Id, ContactName, RepairNumber, ModelNumber, Status, DateSent, DateReceived FROM nodeData WHERE RepairNumber LIKE '%" + getSearchCriteria + "%' AND Status != 'Shipped' ORDER BY rowid DESC;"; }
       else if (getSearchCategory == "Model Number") { dbQuery = "SELECT Id, ContactName, RepairNumber, ModelNumber, Status, DateSent, DateReceived FROM nodeData WHERE ModelNumber LIKE '%" + getSearchCriteria + "%' AND Status != 'Shipped' ORDER BY rowid DESC;"; }
       else if (getSearchCategory == "Status") { dbQuery = "SELECT Id, ContactName, RepairNumber, ModelNumber, Status, DateSent, DateReceived FROM nodeData WHERE Status LIKE '%" + getSearchCriteria + "%' AND Status != 'Shipped' ORDER BY rowid DESC;"; }
       else if (getSearchCategory == "Last Update") { dbQuery = "SELECT Id, ContactName, RepairNumber, ModelNumber, Status, DateSent, DateReceived FROM nodeData WHERE DateSent LIKE '%" + getSearchCriteria + "%' AND Status != 'Shipped' ORDER BY rowid DESC;"; }
       else if (getSearchCategory == "Date Received") { dbQuery = "SELECT Id, ContactName, RepairNumber, ModelNumber, Status, DateSent, DateReceived FROM nodeData WHERE DateReceived LIKE '%" + getSearchCriteria + "%' AND Status != 'Shipped' ORDER BY rowid DESC;"; }
       else { dbQuery = "SELECT Id, ContactName, RepairNumber, ModelNumber, Status, DateSent, DateReceived FROM nodeData WHERE Status != 'Shipped' ORDER BY rowid ASC;"; }
...
void NervoPanel::onClickUpdateRepair(wxCommandEvent& event)
{
    ...
    else
    {
        ...
        NervoupdateRepairDialog* urdObjPtr = new NervoupdateRepairDialog(wxT("Update Repair"));
...
void NervoPanel::onClickReset(wxCommandEvent& event)
{
    dbQuery = "SELECT Id, ContactName, RepairNumber, ModelNumber, Status, DateSent, DateReceived FROM nodeData WHERE Status != 'Shipped' ORDER BY rowid ASC;";
...
void NervoPanel::onClickSort(wxCommandEvent& event)
{
    ...
    if (getSortCriteria == "Newest to Oldest") { dbQuery = "SELECT Id, ContactName, RepairNumber, ModelNumber, Status, DateSent, DateReceived FROM nodeData WHERE Status != 'Shipped' ORDER BY rowid DESC;"; }
    else if (getSortCriteria == "Oldest to Newest") { dbQuery = "SELECT Id, ContactName, RepairNumber, ModelNumber, Status, DateSent, DateReceived FROM nodeData WHERE Status != 'Shipped' ORDER BY rowid ASC;"; }
    else if (getSortCriteria == "Doctor") { dbQuery = "SELECT Id, ContactName, RepairNumber, ModelNumber, Status, DateSent, DateReceived FROM nodeData WHERE Status != 'Shipped' ORDER BY ContactName ASC;"; }
    else if (getSortCriteria == "Repair #") { dbQuery = "SELECT Id, ContactName, RepairNumber, ModelNumber, Status, DateSent, DateReceived FROM nodeData WHERE Status != 'Shipped' ORDER BY RepairNumber ASC;"; }
    else if (getSortCriteria == "Model #") { dbQuery = "SELECT Id, ContactName, RepairNumber, ModelNumber, Status, DateSent, DateReceived FROM nodeData WHERE Status != 'Shipped' ORDER BY ModelNumber ASC;"; }
    else if (getSortCriteria == "Status") { dbQuery = "SELECT Id, ContactName, RepairNumber, ModelNumber, Status, DateSent, DateReceived FROM nodeData WHERE Status != 'Shipped' ORDER BY Status ASC;"; }
    else if (getSortCriteria == "Date Received") { dbQuery = "SELECT Id, ContactName, RepairNumber, ModelNumber, Status, DateSent, DateReceived FROM nodeData WHERE Status != 'Shipped' ORDER BY DateReceived ASC;"; }
    else if (getSortCriteria == "Last Updated") { dbQuery = "SELECT Id, ContactName, RepairNumber, ModelNumber, Status, DateSent, DateReceived FROM nodeData WHERE Status != 'Shipped' ORDER BY DateSent ASC;"; }
    else { dbQuery = "SELECT Id, ContactName, RepairNumber, ModelNumber, Status, DateSent, DateReceived FROM nodeData WHERE Status != 'Shipped' ORDER BY rowid ASC;"; }
...
NervonewRepairDialog::NervonewRepairDialog(const wxString& title)
    : wxDialog(NULL, -1, ("Create New Repair"), wxPoint(1100, 500), wxSize(630, 480))
{
    ...
    wxStaticText* contact = new wxStaticText(panel, wxID_ANY, ("Doctor Name:"), wxPoint(10, 10));
    contact->SetForegroundColour(wxColor(0, 255, 255));
    contact->SetFont(wxFont(14, wxSWISS, wxNORMAL, wxNORMAL, false, _("MS Sans Serif")));

    wxStaticText* company = new wxStaticText(panel, wxID_ANY, ("Practice Name:"), wxPoint(10, 45));
...
NervonewRepairDialog::~NervonewRepairDialog()
{
}
...
void NervonewRepairDialog::onClickCreateRepair(wxCommandEvent& event)
{
   const int MAXLEN = 80;
    char s[MAXLEN];
    time_t t = time(0);
    ...
    else
    {
        ...
        testDBName = wxGetCwd() + wxT("//TemplateDatabase.db");
        db->Open(testDBName);
        db->ExecuteUpdate("CREATE TABLE IF NOT EXISTS 'nodeData' ( 'Id' INTEGER PRIMARY KEY AUTOINCREMENT, 'CompanyName' TEXT, 'ContactName' TEXT, 'Address' TEXT, 'PhoneNumber' TEXT, 'EmailAddress' TEXT, 'Source' TEXT, unique(ContactName,Address));");
...
void NervonewRepairDialog::onClickCancelRepair(wxCommandEvent& event)
{
    Destroy();
}
...
void NervonewRepairDialog::onClickGetRepairTemplate(wxCommandEvent& event)
{
...
NervoupdateRepairDialog::NervoupdateRepairDialog(const wxString& title)
    : wxDialog(NULL, -1, ("Update Existing Repair"), wxPoint(1100, 500), wxSize(1200, 764))
{
  ...
  //  updateRepairDataGrid2->SetCellAlignment(6, 1, wxALIGN_CENTRE, wxALIGN_CENTRE);
    ...
    const wxString RepairLevelsNE[] =
    {
        "C006 - Calibration",
        "C007 - Chrome tubes, replace",
        "C011 - Locking Spring, replace",
        "C012 - Meter, clean & balance",
        "C014 - On switch, replace",
        "C018 - TCs, clean, straighten, refill",
        "C019 - TCs, replace",
        "C020 - TCs, straighten",
        "C023 - Top case, replace",
        "C025 - Black tubes, replace",
        "C026 - Pivot friction job",
        "C027 - Pointer, replace",
        "C028 - Wing nut, replace",
        "C030 - Block, replace",
        "C031 - Meter base, replace",
        "C032 - Adapter for Murdock TCs",
        "C034 - Cut down thermocouples",
        "C035 - Open up front end (for EDL standard or Palmer)",
        "C048 - Old style glass",
        "C051 - Replace meter",
        "NRP"
      
    };

    const wxString RepairLevelsType[] =
    {
  ...
  //  db->ExecuteQuery("PRAGMA foreign_keys = OFF;");
   ...
   Res2.Finalize();
    ...
    wxPanel* panelSTAT = new wxPanel(parentPanel, -1, wxPoint(-1, 600), wxSize(585, 40));
    ...
    wxPanel* panel3 = new wxPanel(parentPanel, -1, wxPoint(600, 40), wxSize(585, 180));
    panel3->SetBackgroundColour(wxColor(100, 100, 200));

    updateRepairDataGrid3 = new wxGrid(panel3, wxID_ANY,
        wxPoint(-1, -1), wxSize(585, 180));
    ...
    updateRepairDataGrid3->SetDefaultRowSize(175);
    ...
    wxPanel* panel6 = new wxPanel(parentPanel, -1, wxPoint(600, 247), wxSize(475, 40));
    panel6->SetBackgroundColour(wxColor(100, 100, 200));

    updateRepairDataGrid6 = new wxGrid(panel6, wxID_ANY,
        wxPoint(-1, -1), wxSize(475, 40));
    ...
    updateRepairDataGrid6->SetColSize(0, 475);
    updateRepairDataGrid6->SetCellEditor(0, 0, new wxGridCellChoiceEditor(WXSIZEOF(RepairLevelsType), RepairLevelsType));
    updateRepairDataGrid6->SetCellValue(0, 0, "Nervo-Scope Type:");


    


    wxPanel* panel8 = new wxPanel(parentPanel, -1, wxPoint(600, 300), wxSize(493, 40));
    ...
    wxBitmapButton* CreateSelectbutton = new wxBitmapButton(parentPanel, idButtonSelect, wxBitmap(wxT("select.png"),
        wxBITMAP_TYPE_PNG), wxPoint(1081, 244), wxSize(-1, -1), wxSIMPLE_BORDER);
    wxImage CreateSelectbuttonimgHover("selecth.png");
    CreateSelectbutton->SetBitmapHover(CreateSelectbuttonimgHover);
    wxImage CreateSelectbuttonimgPressed("selectp.png");
    CreateSelectbutton->SetBitmapPressed(CreateSelectbuttonimgPressed);

    wxBitmapButton* CreateAddbutton2 = new wxBitmapButton(parentPanel, idButtonAddNote, wxBitmap(wxT("add.png"),
        wxBITMAP_TYPE_PNG), wxPoint(1097,295), wxSize(-1, -1), wxSIMPLE_BORDER);
    ...
    wxPanel* panel4 = new wxPanel(parentPanel, -1, wxPoint(600, 347), wxSize(585, 180));
    ...
    updateRepairDataGrid4->SetDefaultRowSize(175);
    ...
    wxPanel* panel5 = new wxPanel(parentPanel, -1, wxPoint(600, 545), wxSize(585, 180));
    ...
    updateRepairDataGrid5->SetDefaultRowSize(175);
    ...
    updateRepairDataGridLVL->SetDefaultRowSize(30);
    ...
    updateRepairDataGridLVL->SetColSize(1, 420);
...
NervoupdateRepairDialog::~NervoupdateRepairDialog()
{
}
...
void NervoupdateRepairDialog::onClickDoRepairUpdate(wxCommandEvent& event)
{
...
void NervoupdateRepairDialog::onClickCancelRepairUpdate(wxCommandEvent& event)
{
    Destroy();
}

void NervoupdateRepairDialog::onClickAddNote(wxCommandEvent& event)
{
...
void NervoupdateRepairDialog::onClickSelect(wxCommandEvent& event)
{
    ...
    const wxString RepairLevelsE[] =
    {
        "C001 - Battery, replace",
        "C002 - Battery cover, replace",
        "C003 - Block, replace",
        "C004 - Bottom board, replace",
        "C005 - Cable coil cord, replace",
        "C006 - Calibration",
        "C007 - Chrome tubes, replace",
        "C008 - Front end cover/plate",
        "C009 - Housing (plastic), replace",
        "C010 - Jack module, replace",
        "C011 - Locking Spring, replace",
        "C012 - Meter, clean & balance",
        "C013 - Meter base, replace",
        "C014 - On switch, replace",
        "C015 - Pointer, replace",
        "C016 - Taut band, replace",
        "C017 - TCs, clean",
        "C018 - TCs, clean, straighten, refill",
        "C019 - TCs, replace",
        "C021 - Thumb screw, replace",
        "C022 - Timing circuit, replace",
        "C023 - Top case, replace",
        "C024 - Top board, replace",
        "C045 - ETS-9 to ETS-9A upgrade**",
        "C046 - New TCs-high sensitivity",
        "C047 - Battery cable",
        "C048 - Old style glass",
        "C049 - Electronic slide switch",
        "C050 - Replace Meter",
        "C000 - Other",
        "NRP - Not Repairable"
    };
    ...
    string nervoScopeType = updateRepairDataGrid6->GetCellValue(0, 0);

    if (nervoScopeType == "Non-Electronic Model")
    {
        updateRepairDataGrid8->SetCellEditor(0, 0, new wxGridCellChoiceEditor(WXSIZEOF(RepairLevelsNE), RepairLevelsNE));
        updateRepairDataGrid8->SetCellValue(0, 0, "NON-ELECTRONIC MODEL:");
    }
    else if (nervoScopeType == "Electronic Model")
    {
        updateRepairDataGrid8->SetCellEditor(0, 0, new wxGridCellChoiceEditor(WXSIZEOF(RepairLevelsE), RepairLevelsE));
        updateRepairDataGrid8->SetCellValue(0, 0, "ELECTRONIC MODEL:");
    }
...
void NervoPanel::OnCellRightClick(wxGridEvent& ev) 
{
...
void NervoPanel::OnCellLeftClick(wxGridEvent& ev) 
{
...
void NervoPanel::OnCellLeftDoubleClick(wxGridEvent& ev)
{
