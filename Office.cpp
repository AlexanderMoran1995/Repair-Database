...
const long OfficePanel::idButtonGetInfo = ::wxNewId();   //Get Button
const long OfficePanel::idButtonSearch = ::wxNewId();   //Search Button
const long OfficePanel::idButtonResetGrid = ::wxNewId();   //Reset Button
const long OfficePanel::idButtonSortGrid = ::wxNewId();   //Sort Button
const long OfficePanel::idButtonExcelOutput = ::wxNewId();
const long OfficePanel::idButtonLogReport = ::wxNewId();
const long OfficePanel::idButtonRepairForm = ::wxNewId();
const long OfficePanel::idButtonRepairTag = ::wxNewId();
const long OfficePanel::idButtonFdaSpreadsheet = ::wxNewId();

BEGIN_EVENT_TABLE(OfficePanel, wxPanel)


EVT_BUTTON(idButtonGetInfo, OfficePanel::onClickGet) //New User Button
EVT_BUTTON(idButtonSearch, OfficePanel::onClickSearch) //Login Button
EVT_BUTTON(idButtonResetGrid, OfficePanel::onClickReset) //Login Button
EVT_BUTTON(idButtonSortGrid, OfficePanel::onClickSort) //Login Button
EVT_BUTTON(idButtonExcelOutput, OfficePanel::onClickExcelOutput)
EVT_BUTTON(idButtonLogReport, OfficePanel::onClickLogReport)
EVT_BUTTON(idButtonRepairForm, OfficePanel::onClickRepairForm)
EVT_BUTTON(idButtonRepairTag, OfficePanel::onClickRepairTag)
EVT_BUTTON(idButtonFdaSpreadsheet, OfficePanel::onClickFdaSpreadsheet)

EVT_TIMER(TIMER_ID, OfficePanel::OnTimer)

EVT_GRID_CELL_LEFT_CLICK(OfficePanel::OnCellLeftClick)
EVT_GRID_CELL_RIGHT_CLICK(OfficePanel::OnCellRightClick)
...
OfficeBackgroundPanel::OfficeBackgroundPanel(wxPanel* parent)
    : wxPanel(parent, -1, wxDefaultPosition,
        wxSize(1207, 707))
{

    OP = new OfficePanel(this);

}


OfficeBackgroundPanel::~OfficeBackgroundPanel() {}

OfficePanel::OfficePanel(wxPanel* parent)
    : wxPanel(parent, -1, wxDefaultPosition,
        wxSize(1207, 707))
{
    ///PARENT PANEL
    ...
    grid->SetColLabelValue(6, _("Status"));
	...
	wxSQLite3ResultSet Res = db->ExecuteQuery("SELECT DISTINCT RepairNumber FROM nodeData ORDER BY Chronology DESC;");
	...
	db->Open(testDBName);
	wxSQLite3ResultSet Res24 = db->ExecuteQuery("SELECT DISTINCT RepairNumber FROM nodeData WHERE Status = 'Shipped' ORDER BY rowid DESC;");
	std::vector<wxString> shippedVector;
	int shippedCount = 0;

	while (Res24.NextRow())
	{
		shippedVector.push_back(Res24.GetAsString(0));
		shippedCount++;
	}
	...
	for (int i = 0; i < repairCount-1; i++)
	{
		for (int j = 0; j < shippedCount-1; j++)
		{

			if (repairVector.at(i) == shippedVector.at(j))
			{
				repairVector.erase(std::remove(repairVector.begin(), repairVector.end(), shippedVector.at(j)), repairVector.end());
				repairCount -= 1;
			}
	...
	for (int i = 0; i < repairCount; i++) {
		 ...
		 Res2 = db->ExecuteQuery("SELECT Id, ContactName, CompanyName, RepairNumber, ModelNumber, Technician, Status, DateSent, DateReceived FROM nodeData WHERE RepairNumber = '" + repairVector.at(i) +"' ORDER BY Chronology DESC LIMIT 1;");
    ...
    wxBitmapButton* LogReportButton = new wxBitmapButton(exportPanel, idButtonLogReport, wxBitmap(wxT("logReport.png"),
        wxBITMAP_TYPE_PNG), wxPoint(134, 100), wxSize(-1, -1), wxSIMPLE_BORDER);
    ...
    wxBitmapButton* RepairFormButton = new wxBitmapButton(exportPanel, idButtonRepairForm, wxBitmap(wxT("repairForm.png"),
        wxBITMAP_TYPE_PNG), wxPoint(134, 150), wxSize(-1, -1), wxSIMPLE_BORDER);
    wxImage RepairFormButtonimgHover("repairFormh.png");
    RepairFormButton->SetBitmapHover(RepairFormButtonimgHover);
    wxImage RepairFormButtonimgPressed("repairFormp.png");
    RepairFormButton->SetBitmapPressed(RepairFormButtonimgPressed);

    wxBitmapButton* RepairTagButton = new wxBitmapButton(exportPanel, idButtonRepairTag, wxBitmap(wxT("repairTag.png"),
        wxBITMAP_TYPE_PNG), wxPoint(134, 200), wxSize(-1, -1), wxSIMPLE_BORDER);
    wxImage RepairTagButtonimgHover("repairTagh.png");
    RepairTagButton->SetBitmapHover(RepairTagButtonimgHover);
    wxImage RepairTagButtonimgPressed("repairTagp.png");
    RepairTagButton->SetBitmapPressed(RepairTagButtonimgPressed);
	...
	wxBitmapButton* fdaSpreadsheetButton = new wxBitmapButton(exportPanel, idButtonFdaSpreadsheet, wxBitmap(wxT("FDAspreadsheet.png"),
		wxBITMAP_TYPE_PNG), wxPoint(81, 250), wxSize(-1, -1), wxSIMPLE_BORDER);
	...
	PDFEntry = new wxTextCtrl(exportPanel, -1, wxT(""), wxPoint(3, 153),
		wxSize(110, 27));
	PDFEntry->SetBackgroundColour(wxColour(228, 228, 240));
	PDFEntry->SetFont(wxFont(12, wxSWISS, wxBOLD, wxBOLD, false, _("MS Sans Serif")));
	

	rpNumEntry = new wxTextCtrl(exportPanel, -1, wxT(""), wxPoint(3, 203),
		wxSize(110, 27));
	rpNumEntry->SetBackgroundColour(wxColour(228, 228, 240));
	rpNumEntry->SetFont(wxFont(12, wxSWISS, wxBOLD, wxBOLD, false, _("MS Sans Serif")));
...
void OfficePanel::onClickGet(wxCommandEvent& event)
{
...
void OfficePanel::onClickSearch(wxCommandEvent& event)
{
    ...
    else
    {
		...
		if (getSearchCategory == "Contact Name") { dbQuery = "SELECT DISTINCT Id, ContactName, CompanyName, RepairNumber, ModelNumber, Technician, Status, DateSent, DateReceived FROM nodeData WHERE ContactName LIKE '%" + getSearchCriteria + "%' AND RepairNumber = '" + repairVector.at(i) + "' ORDER BY Chronology DESC;"; }
        else if (getSearchCategory == "Company Name") { dbQuery = "SELECT DISTINCT Id, ContactName, CompanyName, RepairNumber, ModelNumber, Technician, Status, DateSent, DateReceived FROM nodeData WHERE CompanyName LIKE '%" + getSearchCriteria + "%' AND RepairNumber = '" + repairVector.at(i) + "' ORDER BY Chronology DESC;"; }
        else if (getSearchCategory == "Repair Number") { dbQuery = "SELECT DISTINCT Id, ContactName, CompanyName, RepairNumber, ModelNumber, Technician, Status, DateSent, DateReceived FROM nodeData WHERE RepairNumber LIKE '%" + getSearchCriteria + "%' AND RepairNumber = '" + repairVector.at(i) + "' ORDER BY Chronology DESC;";}
        else if (getSearchCategory == "Model Number") { dbQuery = "SELECT DISTINCT Id, ContactName, CompanyName, RepairNumber, ModelNumber, Technician, Status, DateSent, DateReceived FROM nodeData WHERE ModelNumber LIKE '%" + getSearchCriteria + "%' AND RepairNumber = '" + repairVector.at(i) + "' ORDER BY Chronology DESC;"; }
        else if (getSearchCategory == "Technician") { dbQuery = "SELECT DISTINCT Id, ContactName, CompanyName, RepairNumber, ModelNumber, Technician, Status, DateSent, DateReceived FROM nodeData WHERE Technician LIKE '%" + getSearchCriteria + "%' AND RepairNumber = '" + repairVector.at(i) + "' ORDER BY Chronology DESC;"; }
        else if (getSearchCategory == "Status") { dbQuery = "SELECT DISTINCT Id, ContactName, CompanyName, RepairNumber, ModelNumber, Technician, Status, DateSent, DateReceived FROM nodeData WHERE Status LIKE '%" + getSearchCriteria + "%' AND RepairNumber = '" + repairVector.at(i) + "' ORDER BY Chronology DESC;"; }
        else if (getSearchCategory == "Last Updated") { dbQuery = "SELECT DISTINCT Id, ContactName, CompanyName, RepairNumber, ModelNumber, Technician, Status, DateSent, DateReceived FROM nodeData WHERE DateSent LIKE '%" + getSearchCriteria + "%' AND RepairNumber = '" + repairVector.at(i) + "' ORDER BY Chronology DESC;"; }
        else if (getSearchCategory == "Date Received") { dbQuery = "SELECT DISTINCT Id, ContactName, CompanyName, RepairNumber, ModelNumber, Technician, Status, DateSent, DateReceived FROM nodeData WHERE DateReceived LIKE '%" + getSearchCriteria + "%' AND RepairNumber = '" + repairVector.at(i) + "' ORDER BY Chronology DESC;"; }
		else { dbQuery = "SELECT DISTINCT Id, ContactName, CompanyName, RepairNumber, ModelNumber, Technician, Status, DateSent, DateReceived FROM nodeData WHERE RepairNumber = '" + repairVector.at(i) + "' ORDER BY Chronology DESC;"; }
...
void OfficePanel::onClickReset(wxCommandEvent& event)
{
	RESET();
}


void OfficePanel::OnTimer(wxTimerEvent& event)
{
	RESET();
}
...
void OfficePanel::onClickSort(wxCommandEvent& event)
{
...
bool ChartSample::WriteLogData(wxExcelRange& range)
{
...
bool ChartSample::WriteFdaData(wxExcelRange& range) 
{
	db = new wxSQLite3Database();
...
bool ChartSample::AddTest(wxExcelRange& range)
{
...
void ChartSample::ChartFormat(wxExcelRange& range)
{
	// write sheet headers
	variant.ClearList();
...
void ChartSample::LogFormat(wxExcelRange& range)
{
	// write sheet headers
	variant.ClearList();
...
void ChartSample::FdaFormat(wxExcelRange& range)
{
	// write sheet headers
	variant.ClearList();
	...
	wxSQLite3ResultSet Res2 = db->ExecuteQuery("SELECT RepairNumber,DateReceived,ContactName,ModelNumber,SensorSerialNumber FROM nodeData WHERE ModelNumber IN('ETS-9','ETS-9A' ) ORDER BY rowid ASC;");
...
bool ChartSample::AddTestChart()
{

	wxExcelChart chart;

	chart = m_testChartWorksheet.GetShapes().AddChart(xlColumnClustered, 1, 1, 250, 250).GetChart();
	chart.SetHasTitle(true);
	chart.GetChartTitle().SetText("Test chart");
	m_testChartWorksheet.ChartObjects()[1].SetRoundedCorners(true);

	if (!chart)
		return false;

	wxExcelRange sourceRange = m_dataWorksheet.GetRange("A1:C5");
	if (!sourceRange)
		return false;
	...
	axis.SetHasTitle(true);
	axisTitle = axis.GetAxisTitle();
	axisTitle.SetCaption("Receiving and Updates");
	...
	categories.push_back("1");
	categories.push_back("2");
	//categories.GetFormat().GetLine().GetForeColor().SetRGB(*wxBLUE);
	axis.SetCategoryNames(categories);

	axis.SetHasTitle(true);
	axisTitle = axis.GetAxisTitle();
	axisTitle.SetCaption("Date");
...
void OfficePanel::onClickExcelOutput(wxCommandEvent& event)
{
...
void OfficePanel::onClickLogReport(wxCommandEvent& event)
{
...
void OfficePanel::onClickRepairForm(wxCommandEvent& event)
{
	using namespace std;
	/*
	...
	wxString repairFile = PDFEntry->GetValue();
	...
	PDFNet::Initialize();
					...
					rpFormDate = Res.GetAsString(0);
					rpFormCompany = Res.GetAsString(1);
					rpFormContact = Res.GetAsString(2);
					rpFormEmail = Res.GetAsString(3);
					rpFormPhone = Res.GetAsString(4);
					rpFormAddress = Res.GetAsString(5);
					rpFormNotes = Res.GetAsString(6);
	...
	string output_path = "Template/";
	string output_path2 = "Output/Repair_Form"+ repairFile;

	remove("Output/Repair_Form" + repairFile);
	...
	PDFDoc doc((output_path + "repair sheet form.pdf").c_str());
	doc.InitSecurityHandler();

	
	Field a = doc.GetField("Date Received");
	a.SetValue(rpFormDate);

	Field b = doc.GetField("Received From");
	b.SetValue(rpFormCompany);

	Field c = doc.GetField("Contact");
	c.SetValue(rpFormContact);

	Field d = doc.GetField("Email");
	d.SetValue(rpFormEmail);

	Field e = doc.GetField("Phone");
	e.SetValue(rpFormPhone);

	Field f = doc.GetField("Ship To");
	f.SetValue(rpFormAddress);

	Field g = doc.GetField("Text6");
	g.SetValue(rpFormNotes);
	...
	db->Open(testDBName);
		...
		wxSQLite3ResultSet  Res6 = db->ExecuteQuery("SELECT RepairNumber,Quantity,ModelNumber,SensorSerialNumber,CaseYN,Level FROM nodeData WHERE History = 'CREATED' AND DateReceived = '" + rpFormDate + "' AND CompanyName = '" + rpFormCompany + "' AND ContactName = '" + rpFormContact + "' AND Address = '" + rpFormAddress + "'   ORDER BY Chronology DESC;");
		...
		while (Res6.NextRow())
		{
			rpFormRepairNum = Res6.GetAsString(0);
			rpFormQuantity = Res6.GetAsString(1);
			rpFormModelNum = Res6.GetAsString(2);
			rpFormSerialNum = Res6.GetAsString(3);
			rpFormCase = Res6.GetAsString(4);
			...
			Field h = doc.GetField("Repair" + s);
			h.SetValue(rpFormRepairNum);

			Field i = doc.GetField("Quantity" + s);
			i.SetValue(rpFormQuantity);

			Field j = doc.GetField("Model" + s);
			j.SetValue(rpFormModelNum);

			Field k = doc.GetField("Serial" + s);
			k.SetValue(rpFormSerialNum);

			Field l = doc.GetField("Case" + s);
			l.SetValue(rpFormCase);

			Field m = doc.GetField("Level" + s);
			m.SetValue(rpFormLevel);
	...
	wxString formOutput = "Repair_Form" + repairFile + ".pdf";
	doc.Save((output_path2 + ".pdf").c_str(), 0, NULL);
...
string addressAltered = rpFormAddress;
replace(addressAltered.begin(), addressAltered.end(), ',', '/');

string notesAltered = rpFormNotes;
...
wxSQLite3ResultSet  Res6 = db->ExecuteQuery("SELECT RepairNumber,Quantity,ModelNumber,SensorSerialNumber,CaseYN,Level FROM nodeData WHERE History = 'CREATED' AND DateReceived = '" + rpFormDate + "' AND CompanyName = '" + rpFormCompany + "' AND ContactName = '" + rpFormContact + "' AND Address = '" + rpFormAddress + "'   ORDER BY Chronology ASC;");
...
void OfficePanel::onClickRepairTag(wxCommandEvent& event)
{
	
  
  //if (rpNumEntry == " ")
 // {
	//  wxString msg = _T("ERROR: No Input Detected For 'REPAIR TAG' Please Try again");
	 // wxString info = _T("Repair Tag ERROR");
	//  wxMessageBox(msg,
	//	  info,
	//	  wxOK | wxICON_WARNING,
	//	  this);
 ...
 // {
	...
	wxString tagNumber = rpNumEntry->GetValue();
	...
	db->Close();
	db->Open(testDBName);
	wxSQLite3ResultSet Res = db->ExecuteQuery("SELECT DateReceived, CompanyName, ModelNumber, RepairNumber, SensorSerialNumber, CaseYN, GroupNumber FROM nodeData WHERE RepairNumber ='"+ tagNumber +"' ORDER BY rowid ASC LIMIT 1;");
	
	 rpTagDate = Res.GetAsString(0);
	 rpTagCompany = Res.GetAsString(1);
	 rpTagModelNum = Res.GetAsString(2);
	 rpTagRepairNum = Res.GetAsString(3);
	 rpTagSerialNum = Res.GetAsString(4);
	 rpTagCase = Res.GetAsString(5);
	 rpTagGroup = Res.GetAsString(6);
	...
	if (rpTagDate == "") {
		...
		if (rpTagDate == "") {
			...
			db->Close();
			db->Open(testDBName);
			wxSQLite3ResultSet Res = db->ExecuteQuery("SELECT DateReceived, ContactName, ModelNumber, RepairNumber, SensorSerialNumber, CaseYN, GroupNumber FROM nodeData WHERE RepairNumber ='" + tagNumber + "' ORDER BY rowid ASC LIMIT 1;");
	...
	wxString TagCaseFinal = "Case: " + rpTagCase;

	char szBuf[256];
	char szVbsFile[] = "BcdLabel2.vbs";
	char szItemRepNum[30];
	char szItemDate[30];
	char szItemCompany[30];
	char szItemCase[30];
	char szItemModelNum[30];
	char szItemSerialNum[30];
	char szItemGroupNum[30];


	strcpy(szItemRepNum, rpTagRepairNum.c_str());
	strcpy(szItemDate, rpTagDate.c_str());
	strcpy(szItemCompany, rpTagCompany.c_str());
	strcpy(szItemCase, TagCaseFinal.c_str());
	strcpy(szItemModelNum, rpTagModelNum.c_str());
	strcpy(szItemSerialNum, rpTagSerialNum.c_str());
	strcpy(szItemGroupNum, rpTagGroup.c_str());

	/* Excutes VB-Script via wscript(WSH). */
	sprintf(szBuf, "wscript \"%s\" \"%s\" \"%s\" \"%s\" \"%s\" \"%s\" \"%s\" \"%s\"", szVbsFile, szItemRepNum, szItemDate, szItemCompany, szItemCase, szItemModelNum, szItemSerialNum, szItemGroupNum);
	system(szBuf);
...
void OfficePanel::onClickFdaSpreadsheet(wxCommandEvent& event)
{
...
void OfficePanel::OnCellRightClick(wxGridEvent& ev)
{
...
void OfficePanel::OnCellLeftClick(wxGridEvent& ev)
{
